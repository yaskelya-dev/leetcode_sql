...
Конструкции: WHERE, AND, OR, IS NULL, BETWEEN, LIKE, ~ (регулярки), INTERVAL
...
Конструкции: CASE WHEN, условная агрегация, ROUND(), ::numeric
...
```
3. Обратный разворот таблицы (Unpivot) через CROSS JOIN LATERAL. Берется 1-ая строка товара p. Блок VALUES подставляет значения из этой строки и делает из них микро-табличку из 3 строк. CROSS JOIN прикрепляет эти 3 строки к товару. В итоге 1 строка таблицы трансформируется в 3 строки.
```SQL
